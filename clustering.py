...
class image_clustering:
	...
	def __init__(self, folder_path="/home/mahnaz/wsi-learning/data/icaird_animal/cache/train/256_256_0", n_clusters=4, max_examples=None, use_imagenets= "VGG16" , use_pca=False):
	...
	def load_images(self):
		...
		self.images = np.float32(self.images).reshape(len(self.images), 256, 256, 3)
	...
	def get_new_imagevectors(self):
		if self.use_imagenets == False:
			...
			if use_imagenets.lower() == "vgg16":
				model1 = keras.applications.VGG16(include_top=False, weights="imagenet", input_shape=(256,256,3))
			elif use_imagenets.lower() == "vgg19":
				model1 = keras.applications.VGG19(include_top=False, weights="imagenet", input_shape=(256,256,3))
			elif use_imagenets.lower() == "resnet50":
				model1 = keras.applications.ResNet50(include_top=False, weights="imagenet", input_shape=(256,256,3))
			elif use_imagenets.lower() == "xception":
				model1 = keras.applications.Xception(include_top=False, weights='imagenet',input_shape=(256,256,3))
			elif use_imagenets.lower() == "inceptionv3":
				keras.applications.InceptionV3(include_top=False, weights='imagenet', input_shape=(256,256,3))
			elif use_imagenets.lower() == "inceptionresnetv2":
				model1 = keras.applications.InceptionResNetV2(include_top=False, weights='imagenet', input_shape=(256,256,3))
			elif use_imagenets.lower() == "densenet":
				model1 = keras.applications.DenseNet201(include_top=False, weights='imagenet', input_shape=(256,256,3))
			elif use_imagenets.lower() == "mobilenetv2":
				model1 = keras.applications.MobileNetV2(input_shape=(256,256,3), alpha=1.0, depth_multiplier=1, include_top=False, weights='imagenet', pooling=None)
			else:
...
if __name__ == "__main__":
	...
	data_path = "/home/mahnaz/wsi-learning/data/icaird_animal/cache/train/256_256_0" # path of the folder that contains the images to be considered for the clustering (The folder must contain only image files)

	max_examples = 100 # number of examples to use, if "None" all of the images will be taken into consideration for the clustering
